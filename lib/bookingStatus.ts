...
export function isLive(status: string, startDate: string, endDate: string): boolean {
  ...
  // Completed = POP submitted → live until end date
  if (status === 'completed') return !!(end && now <= end)
  // Other statuses: live within date range
  if (['confirmed', 'active'].includes(status)) return !!(start && end && now >= start && now < end)
  return false
}
